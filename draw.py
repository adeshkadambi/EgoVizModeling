...
def draw_bbox(
    ...
    assert len(pred_boxes) == len(pred_classes) and len(target_boxes) == len(
        target_classes
    ), "pred_boxes and pred_classes must be the same length, and target_boxes and target_classes must be the same length."
    ...
    if len(pred_classes) >= 1:
        img = draw_bounding_boxes(
            img, boxes=pred_boxes, labels=pred_classes, colors=(255, 0, 0), width=4
        )

    # draw target boxes in green if len is > 0
    if len(target_classes) >= 1:
        img = draw_bounding_boxes(
            img, boxes=target_boxes, labels=target_classes, colors=(0, 255, 0), width=4
        )

    return img


def batch_draw_bbox_image_container(
    images: list[ImageContainer], data_folder: str, mapper: dict, save_path: str
):
    ...
    img_paths = [
        path for path in img_paths if "shan" not in path and "unidet" not in path
    ]
    ...
    for image in images:
        ...
        out: torch.Tensor = draw_bbox(
            img_path, pred_boxes, pred_classes, target_boxes, target_classes
        )
